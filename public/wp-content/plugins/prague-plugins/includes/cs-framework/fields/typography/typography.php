...
class CSFramework_Option_typography extends CSFramework_Options {
	...
	public function output() {
		...
		if ( is_array( $google_json = wp_remote_get( CS_URI .'/fields/typography/google-fonts.json' ) ) ) {
			$google_json  = json_decode( $google_json['body'] );
		} else {
			ob_start();
			require plugin_dir_path( __FILE__ ) . '/google-fonts.json';
			$google_json   = ob_get_clean();
			$google_json   = json_decode( $google_json );
		}
