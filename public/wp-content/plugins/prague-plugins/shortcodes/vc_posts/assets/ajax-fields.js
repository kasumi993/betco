function addSelectValue(data,elements){
	...
	for (var k = 0; k < elements.length; k++) {
		...
		var selectedValue = elements[k] == '[name="pix_filter"]' ? $(elements[k]).attr('data-option') : $(elements[k]).attr('data-option').split(','),
			selectOptions = '',
			selectActive = '';

		for (var i = 0; i < data.length; i++) {
			if ( elements[k] == '[name="pix_filter"]' ) {
				if ( selectedValue.indexOf( data[i].meta_key ) >= 0 ) {
	...
	jQuery('.post_type').on('change', function(){
		...
		$.ajax({
		    ...
		    success: function(data){
		    	...
		    	addSelectValue(data,
					['[name="filter_type"]','[name="filter_type_2"]','[name="filter_type_3"]', '[name="pix_filter"]']
		    		);
