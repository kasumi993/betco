...
$jvh('.mew-padding-inputs').on('change', function(e){
	$umargin = $jvh(this).parent();
	var temp = '';
	$umargin.find('.mew-padding-inputs').each(function(input_index, input){
		...
		if(input_value != '')
		{
			if(input_value.match(/^[0-9]+$/))
				input_value += 'px';
			temp += 'padding-'+margin_parameter+':'+input_value+';';
		}
	});
	$umargin.find('.mew-padding-value').val(temp);
});
$jvh('.mew-padding').each(function(index, element){
	$umargin = $jvh(this);
	var ultimate_margin_value = $umargin.find('.mew-padding-value').val();
